...
<div id="userbar" class="navbar navbar-default" role="navigation">
...
<?php } ?>
	 ...
	 <a type="button" id="login" class="btn btn-primary" href="http://mediawikibootstrap.org/index.php?title=Special:UserLogin&returnto=Main_Page"><i class="fa fa-folder-open"></i> Login</a>
	 <a type="button" id="logout" class="btn btn-primary" href="http://mediawikibootstrap.org/index.php?title=Special:UserLogout"><i class="fa fa-folder-open"></i> Logout</a>
	 <div class="btn-group">
     <button type="button" class="btn btn-primary">Sidebar</button>
     <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown"><i class="fa fa-angle-down"></i></button>
     <ul class="dropdown-menu text-center primary-border" role="menu" style="margin: 0; padding: 8px;">
     <?php
     ...
     <ul class="dropdown-menu text-center primary-border" role="menu" style="margin: 0; padding: 8px;">
     	<?php
    	$rss = new DOMDocument();
    	$rss->load('http://mediawikibootstrap.org/index.php?title=Quick_Links&action=feed&feed=rss');
    	$feed = array();
    ...
    ?> 	
     ...
     <ul class="dropdown-menu text-center primary-border" role="menu" style="margin: 0; padding: 8px;">
     	<?php
    	$rss = new DOMDocument();
    	$rss->load('http://mediawikibootstrap.org/index.php?title=Categories&action=feed&feed=rss');
    	$feed = array();
    	foreach ($rss->getElementsByTagName('item') as $node) {
    		$item = array ( 
    			'title' => $node->getElementsByTagName('title')->item(0)->nodeValue,
...
?>
                  <div class="nav logo-container" role="navigation" id="p-logo" style="margin-right: 20px;margin-top:5px;"><a href="<?php echo htmlspecialchars( $this->data['nav_urls']['mainpage']['href'] ) ?>" <?php echo Xml::expandAttributes( Linker::tooltipAndAccesskeyAttribs( 'p-logo' ) ) ?>><img src="<?php $this->text( 'logopath' ); ?>" alt="<?php $this->html('sitename'); ?>"></a><div>
<?php
  }
  ...
  private function renderNavigation( $elements ) {
    ...
    foreach ( $elements as $name => $element ) {
      ...
      switch ( $element ) {
        ...
        case 'EDIT':
          ...
          if ($navTemp) { ?>
            <div class="actions pull-left nav">
                <a id="b-edit" style="margin-top:1rem;" href="<?php echo $navTemp['href']; ?>" class="btn"><i class="icon-edit"></i> <?php echo $navTemp['text']; ?></a>
            </div>
          <?php } 
        break;
        ...
        case 'SEARCH':
          ?>
            <form style="margin-top: -0.5rem" class="navbar-form navbar-right" action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
              ...
              <?php echo $this->makeSearchButton( 'fulltext', array( 'id' => 'mw-searchButton', 'class' => 'btn btn-default' ) ); ?>
            </form>
